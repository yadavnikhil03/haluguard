...
> = {
  critical: { label: "CRIT", tag: (s) => picocolors.bgRed(picocolors.white(s)), icon: "⛔" },
  ...
  info: { label: "INFO", tag: (s) => picocolors.gray(s), icon: "ℹ️" },
};
...
export function renderReport(report: ScanReport): string {
  ...
  lines.push(
    picocolors.bold(
      picocolors.red(
        `\n  🛡️  HaluGuard found ${report.findings.length} issue${report.findings.length === 1 ? "" : "s"}\n`,
      ),
    ),
  );
